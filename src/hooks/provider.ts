...
import { PROVIDER_EVENTS } from '@/const'
...
export function useProvider<T extends keyof Record<string, string>>() {
  ...
  const init = useCallback(
    async (providerProxy: ProviderProxyConstructor, createProviderOpts?: CreateProviderOpts<T>) => {
      const initializedProvider = await createProvider(providerProxy, createProviderOpts)

      setProvider(prev => (isEqual(initializedProvider, prev) ? prev : initializedProvider))

      setRawProvider(prev =>
        isEqual(initializedProvider?.rawProvider, prev) ? prev : initializedProvider?.rawProvider,
      )
