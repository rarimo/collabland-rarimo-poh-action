...
import { EnvType, handleFetchResponse, sleep, stringify } from '@collabland/common'
import {
...
export class HelloActionController extends BaseDiscordActionController<APIChatInputApplicationCommandInteraction> {
  ...
  async getMetadata(): Promise<DiscordActionMetadata> {
    return {
      ...
      manifest: new MiniAppManifest({
        ...
        supportedEnvs: [EnvType.QA, EnvType.TEST, EnvType.PROD, EnvType.DEV, EnvType.STAGING],
        name: 'HelloAction',
        platforms: ['discord'],
  ...
  protected async handle(
    ...
    logger.debug('User permissions: %O', userPerms)
    ...
    if (userPerms != null) {
      ...
      if (apiToken != null && interaction.actionContext?.callbackUrl != null) {
        ...
        logger.debug('User profile: %O', user)
        const task = async () => {
          await sleep(1000)
          ...
          embeds: [
            {
              ...
              fields: [
                {
                  name: 'user',
                  value: userPerms.user?.username + '#' + userPerms.user?.discriminator,
                },
                {
                  name: 'action',
    ...
    return [
      // `/hello-action <your-name>` slash command
      {
        metadata: {
          name: 'HelloAction',
          shortName: 'hello-action',
          supportedEnvs: [EnvType.DEV, EnvType.QA, EnvType.STAGING],
        },
        name: 'hello-action',
