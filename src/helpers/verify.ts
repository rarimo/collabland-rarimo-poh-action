import { AnyType, debugFactory, getFetch, handleFetchResponse, HttpErrors } from '@collabland/common'
...
export class SignatureVerifier {
  ...
  static async initVerifier() {
    ...
    const keys = await handleFetchResponse<CollabLandConfig>(keysResponse, 200, {
    ...
    SignatureVerifier.ED25519PublicKey = Buffer.from(decode(keys.actionEd25519PublicKey)).toString(
      'hex',
    )
    debug('API URL for Collab.Land Config:', apiUrl)
    debug('SignatureVerifier Initialized')
  }
  ...
  private verifyRequest(
    ...
    if (delta >= 5 * 60 * 1000) {
      throw new HttpErrors[403]('Invalid request - signature timestamp is expired.')
    }
  ...
  private verifyRequestWithEd25519(publicKey: string, signature: string, body: string) {
    let verified = false
    try {
    ...
    if (!verified) {
      throw new HttpErrors[403]('Invalid request - Ed25519 signature cannot be verified.')
    }
    return verified
  }

  private verifyRequestWithEcdsa(publicKey: string, signature: string, body: string) {
    let verified = false
    try {
    ...
    if (!verified) {
      ...
      throw new HttpErrors[403]('Invalid request - Ecdsa signature cannot be verified.')
    }
    return verified
  }
}
